...
- [x] ~~**Browse by Category Cards (P0)**~~ **Done.** — Horizontal scrollable row of 8 category image cards above the catalog grid. Tapping filters. Reuses existing photos. Matches pitch deck Slide 6. (SPEC-012)
...
- [x] ~~**Status Change Notifications (P1)**~~ **Done.** — Toast notifications on view-switch from Equipment Services to Field View. Compares current requests against `prevRequestsRef` snapshot. Shows ✅/🚛 status messages. Auto-dismiss after 5 seconds. Stacked toasts with × close button. Navy left border, bottom-right desktop / bottom-center mobile. Implemented in `page.tsx`. (SPEC-008)
