...
└── components/
    ...
    ├── FieldView.tsx         ← Asset catalog grid with search/status filter + Browse by Category grid (15 types)
...
- [x] ~~**Substitute Recommendations (P0)**~~ **Done.** (SPEC-002)
...
- [x] ~~**Browse by Category Cards (P0)**~~ **Done (updated).** — Responsive CSS grid of all 15 category image cards above the catalog grid. Type dropdown removed from filter bar — Browse by Category is the sole type filtering mechanism. Grid layout: 4 columns on mobile, 5 on tablet, 8 on desktop. Cards use `aspect-ratio: 4/3` for consistent image sizing and `white-space: nowrap` labels (10px mobile, 11px tablet+). Collapsed state shows 1 row; “View all” expands to show all 15 categories; “Show less” collapses back. Tapping a card toggles that category filter. Matches pitch deck Slide 6. (SPEC-012)
...
- [x] ~~**Dispatch Queue Grouping (P1)**~~ **Done.** — Pending requests grouped by time horizon: “Today,” “This Week,” “Future” with colored left-border headers and count badges. Sorted by start date within each group. Empty groups hidden. `DEMO_TODAY` constant set to 2026-07-02 to match seed data window. Active/History tabs unchanged. (SPEC-007)
- [x] ~~**Status Change Notifications (P1)**~~ **Done (updated).** — Toast notifications fire immediately when Equipment Services accepts or declines a request — no tab switch required. Toast generated inside `updateRequestStatus` in `page.tsx`. `prevRequestsRef` removed (no longer needed). Auto-dismiss after 5 seconds. Stacked toasts with × close button. Navy left border, bottom-right desktop / bottom-center mobile. (SPEC-008)
...
- [x] ~~Add maintenance schedule visibility to Equipment Services view~~ **Done.** (SPEC-016)
...
- [x] ~~**Substitute Recommendations**~~ **Done.** (SPEC-002)
...
- [x] ~~**Status Change Notifications**~~ **Done (updated).** — Toasts fire immediately on accept/decline. (SPEC-008)
...
- [x] ~~**Cost Savings Ticker**~~ **Done.** — "ESOP Value Protected This Quarter" banner on Equipment Services Dashboard between KPI strip and Certification Compliance. Calculates savings from fulfilled requests: `days * rate * 0.8` (external rental markup 1.8x). Navy gradient background, large dollar figure (28px mobile / 36px desktop), request count subtitle. Hidden when zero fulfilled requests. Number grows live as ES accepts requests during demo. Dollar icon hidden on mobile for clean layout. (SPEC-019)
...
*Last updated: June 30, 2026. SPEC-019 (cost savings ticker) implemented — "ESOP Value Protected This Quarter" banner on Equipment Services Dashboard; calculates savings from fulfilled requests using 1.8x external markup; navy gradient, large dollar figure, responsive (icon hidden on mobile); number grows live as ES accepts requests. SPEC-018 (add new equipment) implemented — "+Add Equipment" form on Fleet tab; assets lifted into React state with localStorage persistence; CERT_TYPES added; storage.ts extended; assets prop threaded through FieldView, EquipServicesView, FleetOverview; 2-column responsive form with dynamic specs builder; Submit Green button; success toast. All features use mock data per hackathon rules (no auth).*
