...
| **Demo-Safe Persistence (localStorage)** | Wrap application state (requests array, next request number) in `localStorage` so that a browser refresh during the demo does not lose newly created requests. Hide persistence behind a simple service abstraction so a real API can replace it later. | Risk mitigation: if the presenter accidentally refreshes during the hackathon demo, all new requests vanish and the demo breaks. This costs ~20 minutes to implement and eliminates a real failure mode. For production, replace with API-backed persistence. |
...
| **Status Change Notifications** | Display a toast/banner notification when a request status changes (e.g., "Your request REQ-008 was accepted — CAT 320 arriving July 3"). For demo, trigger on view-switch. For production, implement as browser push notifications so field users get updates without having the app open. | Closes the communication loop that currently requires Equipment Services to call the field supervisor back. Without this, users must manually check My Requests. |
...
| **Structured Decline / Delay Reasons** | Upgrade the existing decline modal from free-text only to a reason code dropdown (Maintenance, Unavailable Date, Operator Certification Issue, Transport Constraint, Better Substitute Available) plus optional notes. Store the reason code on the request object alongside the free-text field. | Turns operational pain into useful data. Reason codes enable future analytics on why requests fail, which assets cause the most declines, and where process improvements are needed. Nearly free to implement since the decline modal already exists. |
| **Request Operator Service Option** | Add a "Sundt-provided operator requested" toggle to the Services step of the Order Wizard, alongside the existing fueling configuration. When selected, the request card in Equipment Services highlights the operator requirement. Especially relevant for cranes, excavators, and specialty assets. | Mirrors the real-world workflow where many assets (especially cranes) require a Sundt operator. The data model already tracks operator certification and assignment. Adding the toggle follows the same pattern as the fueling service option. |
| **Dispatch-Oriented Queue Grouping** | Add date-based grouping headers to the Equipment Services request queue: "Today," "This Week," "Future." Sort within each group by requested start date. Optionally allow sorting by yard, job site, or certification risk. | The current flat list works for 7 seed requests but won't scale. Even basic grouping makes the dashboard feel like a real dispatch tool. For production, add multi-column sorting and filtering. |
...
- Demo-safe persistence via localStorage (prevents data loss on browser refresh; replace with API for production)

**P1 — High-Value:**
- Interactive yard & job site map with asset location pins
- Status change notifications (toast on view-switch for demo, push notifications for production)
...
- Structured decline / delay reason codes (Maintenance, Unavailable Date, Cert Issue, Transport, Better Substitute)
- Request Operator service option in Order Wizard ("Sundt-provided operator requested" toggle)
- Dispatch-oriented queue grouping in Equipment Services (Today / This Week / Future)
